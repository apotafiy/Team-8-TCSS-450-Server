...
/**
 * @api {get} /contacts Get contacts for a given memberid
 * @apiName GetContacts
 * @apiGroup Contacts
 *
...
router.get('/', (request, response) => {
  let query =
    'SELECT MemberID, FirstName, LastName, Username FROM Members WHERE MemberID IN (SELECT MemberID_A FROM Contacts WHERE MemberID_B = $1 AND Verified = 0) OR MemberID IN (SELECT MemberID_B FROM Contacts WHERE MemberID_A = $1 AND Verified = 0)';
 ...
 *
 * @apiHeader {String} email of person to be added as a contact
 *
    ...
    pool
      ...
      .catch((err) => {
        response.status(403).send({
          message: 'SQL Error on insert.', // TODO: change this to user not found
        });
      });
  },
  (request, response, next) => {
